...
Now PHP doesn't complain anymore and since interfaces also work with [type hinting]({{site.url}}/chapters/more_on_functions/#type-hinting), any part of our application that expects a `Document` can now use it as a type hint without knowing about all the different kinds of documents. This way, we could later add more classes that implement the `Document` interface and the functions that are type hinted with it will still work as long as they as well adhere to the "contract" that is set up by the interface.
...
The [visibility modifiers]({{site.url}}chapters/objects_and_classes/#visibility) `public`, `protected` and `private` play an important role in inehritance in PHP.
